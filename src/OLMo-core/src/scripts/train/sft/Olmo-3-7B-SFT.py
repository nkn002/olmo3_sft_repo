...
def get_default_attn_backend() -> AttentionBackendName:
    if has_flash_attn_2():
        return AttentionBackendName.flash_2

    log.warning("flash-attn 2 is unavailable, falling back to the PyTorch attention backend")
    ...
    def build(
        ...
        *,
        ...
        try:
            ...
            if require_beaker:
                raise
            # Allow config validation without Beaker credentials.
            root_dir = os.environ.get("OLMO_SFT_ROOT_DIR", "/tmp/olmo-core")
            user_name = os.environ.get("USER", "local")
            print(
                f"Beaker is unavailable; using local root_dir={root_dir!r} for non-launch command."
            )
        else:
            if require_beaker:
        ...
        attn_backend = get_default_attn_backend()
        intra_document_masking = attn_backend != AttentionBackendName.torch
        if not intra_document_masking:
            log.warning(
                "Disabling intra-document masking because the PyTorch attention backend does not support it"
            )
        ...
        config = SFTConfig(
            ...
            trainer=TrainerConfig(
                save_folder=f"{root_dir}/checkpoints/{(user_name or os.environ.get('USER', 'local'))}/olmo-sft/{run_name}",
...
""",
    ...
    config = SFTConfig.build(
        script=sys.argv[0],
        cmd="train",
        require_beaker=args.cmd == "launch",
